...
// মোবাইলের স্ক্রিন অনুযায়ী responsive
...
const brickRows = 6;
const brickCols = 8;
const brickWidth = Math.floor(canvas.width / brickCols - 10);
...
function drawPowerups() {
  powerups.forEach(p => {
    ...
    ctx.fillText(icon, Math.round(p.x - 9), Math.round(p.y + 16));
  });
}
...
function gameLoop() {
  ...
  ctx.shadowColor = "#0cc";
  ctx.shadowBlur = 15;
  ctx.fillRect(Math.round(paddleX), Math.round(paddleY), paddleWidth, paddleHeight);
  ctx.shadowBlur = 0;
  ...
  balls.forEach((ball, bi) => {
    ...
    ctx.beginPath();
    ctx.arc(Math.round(ball.x), Math.round(ball.y), ballRadius, 0, Math.PI * 2);
    ctx.fill();
    ctx.closePath();
  });
  ...
  bricks.forEach(br => {
    ...
    ctx.fillRect(Math.round(br.x), Math.round(br.y), br.width, br.height);
  });
